<?php

return [

    'app' => 'E-Contract App',

    'search' => 'No contract found',
    'waitContract' => 'Work on documenting the contract is still ongoing',
    'success' => 'Success Operation',
    'error' => 'Faild Operation, Something Wrong',
    'verification_success' => 'Account has been activated successfully',
    'verification_faild' => 'Verification code is not correct, try again',
    'verified_account' => 'This account has been verified before',
    'credentials_error' => 'Incorrect mobile or password',
    'unverified_account' => 'You account is not verified, please verify you account',
    'block_account' => 'You account has been blocked, contact with our support',
    'login_success' => 'Login Successfully',
    'send_reset_password_code_failed' => 'send reset password code failed',
    'logout_success' => 'Logout Successfully',
    'otp_code' => 'Login OTP is',
    'send_otp_success' => 'The verification code has been sent on your email',
    'wrong_otp' => 'The verification code is not correct',
    'send_reset_password_code_success' => 'The reset password verification code has been sent on your email',
    'valid_code_to_reset_password' => 'The code is valid, you can reset your password now',
    'wrong_code_to_reset_password' => 'The reset password code is not correct',
    'unauthorized' => "You havn't access",

    'completed_contract' => 'The contract is completed, edit faild',
    'have_uncompleted_contract' => 'There is an uncompleted contract. You have to complete the existing contract to be able to create a new one',
    'have_not_uncompleted_contract' => 'There are no uncompleted contracts',

    'deliver_order_success' => 'You order has been delivered successfully',
    'receive_order_success' => 'You order has been received successfully',

    'empty_cart' => 'The cart is empty',

    'can_not_rate' => 'You can not rate because you did not receive the order',
    'legel_owner' => 'The add legal agent of owner field must be true when property owner is deceased.',
    'status_payment' => 'The payment process has been completed and the payment finalization is being monitored',
    'status_payment_now' => 'Payment status now',
    'associated_with_any_payment' => 'Not payment yet',
    'city_not_include_region' => 'The city does not belong to the region',
    'created_success' => 'Created successfully',
    'updated_success' => 'Updated successfully',
    'updated_successfully' => 'Updated successfully',
    'already_exists' => 'Already exists',
    'failure' => 'Operation failed',
    'blog_not_found' => 'Blog not found',
    'retrieve_blog' => 'Blog retrieved successfully',
    'roles' => 'Roles',
    'permissions_assigned_successfully' => 'Permissions assigned successfully',
    'cannot_delete_role_with_employees' => 'Cannot delete this role because it has employees',
    'contract_period_created_successfully' => 'Contract period created successfully',
    'contract_period_updated_successfully' => 'Contract period updated successfully',
    'contract_period_deleted_successfully' => 'Contract period deleted successfully',
    'contract_period_not_found' => 'Contract period not found',
    'error_deactivating' => 'An error occurred while deactivating',
    'error_sending_sms' => 'Failed to send SMS',
    'payment_not_found' => 'Payment not found',
    'real_estate_not_found' => 'Real estate not found',
    'units' => 'Units',
    'real_estate' => 'Real estate',
    'not_have_real' => 'No real estate found',
    'have_real' => 'Units linked to the real estate',
    'not_have_unit' => 'No units found',
    'not_found' => 'Not found',
    'not_accept' => 'Not allowed',
    'success_remove' => 'Profile deleted successfully',
    'profile_not_exist' => 'Profile does not exist',

    'error_occurred' => 'An error occurred',
    'created_successfully' => 'Created successfully',
    'marketing_report_emailed' => 'The report was sent to your email',
    'deleted_successfully' => 'Deleted successfully',
    'employee_created_successfully' => 'Employee created successfully',
    'employee_updated_successfully' => 'Employee updated successfully',
    'employee_deleted_successfully' => 'Employee deleted successfully',
    'employee_not_found' => 'Employee not found',
    'employee_status_updated' => 'Employee status updated',
    'employee_blocked_successfully' => 'Employee blocked successfully',
    'employee_unblocked_successfully' => 'Employee unblocked successfully',
    'employee_inactive' => 'This employee account is inactive',
    'employee_account_blocked' => 'This employee account is blocked',
    'employee_salary_created_successfully' => 'Employee salary added successfully',
    'employee_note_created_successfully' => 'Employee note added successfully',
    'coupon_created_successfully' => 'Coupon created successfully',
    'coupon_updated_successfully' => 'Coupon updated successfully',
    'coupon_deleted_successfully' => 'Coupon deleted successfully',
    'coupon_inactivated_successfully' => 'Coupon deactivated successfully',
    'coupon_activated_successfully' => 'Coupon activated successfully',
    'coupon_already_deleted' => 'Coupon is already deleted',
    'salary_total_mismatch' => 'Total must equal basic salary minus deduction plus bonus (expected :expected)',

    'contract_updated_successfully' => 'Contract updated successfully',
    'contract_update_requires_field' => 'Send at least one field to update the contract',
    'contract_not_found' => 'Contract not found',
    'received_contract_not_found' => 'No received-contract record for this contract',
    'received_contract_update_requires_field' => 'Send at least one of: status, date_of_received, notes.',
    'contract_received_successfully' => 'Contract received successfully',
    'contract_already_received' => 'The contract has already been received',
    'refund_request_created' => 'Refund request submitted successfully',
    'invalid_draft_contract_number' => 'Invalid draft contract number',
    'refund_contract_id_required' => 'contract_id or draft_contract_number is required',
    'refund_contract_must_be_return_status' => 'Contract must be in the returned orders list — receive the contract first',
    'refund_contract_already_returned' => 'Contract is already returned',
    'return_contract_accepted_successfully' => 'Return contract accepted successfully',
    'return_contract_rejected_successfully' => 'Return contract acceptance rejected successfully',
    'return_contract_status_requires_field' => 'accept_retrun_contract is required (true or false)',
    'refund_request_already_exists' => 'A pending refund request already exists for this contract',
    'refund_update_requires_field' => 'Send at least one of: admin_confirmed, refund_amount, notes',
    'refund_not_found' => 'Refund request could not be identified',
    'refund_already_approved' => 'This refund request was already approved',
    'refund_already_rejected' => 'This refund request was already rejected',
    'refund_amount_invalid' => 'Invalid refund amount',
    'refund_approved_successfully' => 'Management approval recorded successfully',
    'refund_rejected_successfully' => 'Rejection recorded successfully',
    'refund_retracted_successfully' => 'Return request retracted and order reclassified as completed',
    'refund_request_required_for_return_status' => 'A refund request must exist before setting return status',
    'order_not_in_return_status' => 'Order is not in return status',
    'refund_not_found' => 'Refund request could not be identified',
    'refund_already_approved' => 'This refund request was already approved',
    'refund_already_rejected' => 'This refund request was already rejected',
    'refund_amount_invalid' => 'Invalid refund amount',
    'refund_approved_successfully' => 'Management approval recorded successfully',
    'refund_rejected_successfully' => 'Rejection recorded successfully',
    'refund_retracted_successfully' => 'Return request retracted and order reclassified as completed',
    'refund_request_required_for_return_status' => 'A refund request must exist before setting return status',
    'order_not_in_return_status' => 'Order is not in return status',
    'payment_type_in_use' => 'Cannot delete this payment method because it is linked to contracts',
    'contract_period_not_set_for_payment' => 'Payment is not available until contract period (step 6) is completed',
    'contract_payment_amount_invalid' => 'Invalid payment amount — complete contract details and contract period first',
    'contract_already_paid' => 'This contract has already been paid',
    'payment_link_unavailable' => 'Payment link is not available for this record',
    'tenant_role_not_found' => 'Tenant role not found',
    'tenant_role_in_use' => 'Cannot delete tenant role because it is linked to contracts',
    'real_estate_required_for_units' => 'Contract must be linked to a real estate before adding units',
    'contract_unit_not_found' => 'Unit not linked to this contract',
    'contract_units_synced_successfully' => 'Contract units updated successfully',
    'contract_unit_added_successfully' => 'Unit added to contract successfully',
    'contract_unit_updated_successfully' => 'Contract unit updated successfully',
    'contract_unit_removed_successfully' => 'Unit removed from contract successfully',

    'user_blocked_successfull' => 'User blocked successfully',
    'user_unblocked_successfull' => 'User unblocked successfully',
    'user_not_found' => 'User not found',
    'user_deleted_successfull' => 'User deleted successfully',

    'discount_applied_successfully' => 'Discount/waiver applied successfully',
    'discount_contract_not_owned' => 'Contract was not found or does not belong to this client',
    'discount_contract_already_paid' => 'Cannot apply a discount to a paid/completed contract',
    'discount_contract_missing_uuid' => 'Cannot apply a discount because the contract has no uuid',
    'discount_percentage_invalid' => 'Percentage must be greater than 0 and at most 100',
    'discount_fixed_invalid' => 'Fixed discount must be greater than 0',
    'discount_exceeds_contract_total' => 'Discount cannot exceed the contract total',
    'discount_coupon_already_applied' => 'A coupon is already applied to this contract',
    'property_has_contracts' => 'Cannot delete this property because it is linked to contracts',
    'unit_not_found' => 'Unit not found',
    'unit_has_contracts' => 'Cannot delete this unit because it is linked to contracts',
    'deed_not_found' => 'Deed file was not found',
    'ad_spend_imported' => 'Ad spend imported successfully',
    'ad_spend_synced' => 'Ad spend sync finished',

    'user_coupon_created_successfully' => 'Discount saved and secret coupon code created',
    'user_coupon_not_found' => 'Customer coupon was not found',
    'user_coupon_deactivated_successfully' => 'Customer coupon deactivated',
    'user_coupon_not_for_user' => 'This coupon is not assigned to your account',
    'user_coupon_wrong_contract_type' => 'This coupon does not apply to this contract type',
    'user_coupon_expired' => 'Coupon is expired or inactive',
    'user_coupon_code_taken' => 'Secret code is already in use',
    'user_coupon_login_title' => 'Special discount',
    'user_coupon_default_message' => 'Congratulations! You received a special discount on first-year fees.',
    'user_coupon_ack_success' => 'Notification acknowledged',

    'seo_crawl_started' => 'Site scan started',
    'seo_crawl_stopped' => 'Site scan stopped',
    'seo_crawl_in_progress' => 'A site scan is already running',
    'seo_crawl_not_running' => 'No site scan is currently running',
    'seo_crawl_not_found' => 'No crawl results found',

    'google_seo_redirect' => 'Open Google to connect Search Console and Analytics',
    'google_seo_disconnected' => 'Google Search Console disconnected',
    'google_seo_not_configured' => 'GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET are missing',
    'google_seo_invalid_state' => 'Google login expired. Start connect again',
    'google_seo_no_token' => 'Google did not return a token. Grant access and try again',
    'google_seo_not_connected' => 'Connect a Google account first',
    'google_search_console_scope_missing' => 'Search Console access was not granted. Connect Google again',
    'google_search_console_site_missing' => 'Select a Search Console site first',
    'google_search_console_site_invalid' => 'That site is not in the connected Search Console account',
    'google_search_console_site_saved' => 'Search Console site saved',
    'google_search_console_failed' => 'Could not load Search Console data',
    'website_closed' => 'The website is temporarily closed for maintenance',
    'mobile_app_closed' => 'The mobile app is temporarily closed for maintenance',
    'app_update_required' => 'Please update the app to continue',

];

