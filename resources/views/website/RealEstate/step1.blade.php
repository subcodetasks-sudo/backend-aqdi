...
@section('content')
    ...
    <p class="description-state">قم بتعبئة بياناتك بشكل صحيح</p>
         <form action="{{ route('storeStepOne.realEstate', ['id' => $realEstate->id]) }}" method="POST" class="form-content-state">
        ...
        <div class="form-group-state">
            ...
            <select id="ownership-state" class="select-state" name="contract_ownership">
                <option value="" disabled selected>
                    هل أنت المـالك أو المستاجر؟
                </option>
                <option value="1"
                    {{ old('contract_ownership', $realEstate->contract_ownership) == 'owner' ? 'selected' : '' }}>مالك</option>
                <option value="0"
                    {{ old('contract_ownership', $realEstate->contract_ownership) == 'tenant' ? 'selected' : '' }}>مستأجر
                </option>
        ...
        <div class="form-group-state">
            ...
            <select id="alive-state" class="select-state" name="property_owner_is_deceased">
                <option value="" hidden disabled selected {{ old('property_owner_is_deceased', $realEstate->property_owner_is_deceased) === null ? 'selected' : '' }}>هل صاحب العقار حي يرزق؟</option>
                <option value="0" {{ old('property_owner_is_deceased', $realEstate->property_owner_is_deceased) == '0' ? 'selected' : '' }}>هل صاحب العقار حي يرزق</option>
                ...
                <option value="0" {{ old('property_owner_is_deceased', $realEstate->property_owner_is_deceased) == '3' ? 'selected' : '' }}>لا، متوفي يرحمه الله</option>
            </select>
        </div>
        ...
        <div class="form-group-state" id="dead-type-group" style="display: none">
            ...
            <select id="dead-type" class="select-state" name="instrument_type">
                ...
                <option value="old_handwritten" {{ old('instrument_type') == 'old_handwritten' ? 'selected' : '' }}>صك ملكية ورقي (قديم)</option>
                <option value="electronic" {{ old('instrument_type') == 'electronic' ? 'selected' : '' }}>   صك ملكية إلكتروني  </option>
                <option value="strong_argument" {{ old('instrument_type') == 'strong_argument' ? 'selected' : '' }}>صك السجل
                    العقاري</option>
        ...
        <div class="form-group-state input-doc" style="display: none">
            ...
            <input id="document-number" class="input-state" type="number" name="instrument_number" placeholder="رقم الصك"
                value="{{ old('instrument_number') }}" />
        </div>
        ...
        <div class="form-group-state input-doc" style="display: none">
            ...
            <input id="deed-date" class="input-state date-picker" type="text" name="instrument_history"
                value="{{ old('instrument_history') }}" placeholder="تاريخ الصك" />
        </div>
        ...
        <div class="form-group-state document-input" style="display: none">
            ...
            <input id="document-number-registry" class="input-state" type="number" name="real_estate_registry_number"
                value="{{ old('real_estate_registry_number') }}" placeholder="رقم السجل العقاري" />
        </div>
        ...
        <div class="form-group-state document-input" style="display: none">
            ...
            <input id="document-date" class="input-state date-picker" type="text" name="date_first_registration"
                value="{{ old('date_first_registration') }}" placeholder="تاريخ التسجيل الاول" />
        </div>
        ...
        <div class="row-state" style="display: none">
            <div class="form-group-state">
                ...
                <select id="floors-number" class="select-state" name="number_of_floors">
                     <option value="" disabled {{ old('number_of_floors') == '' ? 'selected' : '' }}>عدد الطوابق في
                        العقار</option>
                    @for ($i = 1; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ old('number_of_floors') == $i ? 'selected' : '' }}>
                            {{ $i }}
                        </option>
                    @endfor
                </select>
            ...
            <div class="form-group-state">
                ...
                <select id="units-number" class="select-state" name="number_of_units_in_realestate">
                    <option value="" disabled {{ old('number_of_units_in_realestate') == '' ? 'selected' : '' }}>عدد الوحدات
                        الموجودة في العقار</option>
                    @for ($i = 1; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ old('number_of_units_in_realestate') == $i ? 'selected' : '' }}>
                            {{ $i }}</option>
                    @endfor
        ...
        <div class="modal-content-died">
            ...
            <div class="modal-body-died">
                ...
                <a href="https://wa.me/your-number" class="whatsapp-link-died">
                    ستضاف رسوم بقيمة {{ $valueContract->valueContract ?? 150 }} ريال لمعالجة صك المتوفي
