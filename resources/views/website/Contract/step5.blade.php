...
<form method="POST" action="{{ route('submit.step5', ['uuid' => $contract->uuid]) }}" class="form-content-state" id="owner-form">
    ...
    <div class="row-state">
        <div class="form-group-state">
            ...
            <select id="unit-type" class="select-state" required name="unit_type_id">
                ...
                <!--<option value="0" @if(old('split_ac', $contract->split_ac) == 0) selected @endif>لا يوجد</option>-->

                @foreach ($unitType as $item)
                    <option value="{{ $item->id }}" @if(old('unit_type_id', $contract->unit_type_id) == $item->id) selected @endif>{{ $item->name_trans }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group-state">
            ...
            <select id="unit-usage" class="select-state" required name="unit_usage_id">
                ...
                @foreach ($unitUsage as $item)
                    <option value="{{ $item->id }}" @if(old('unit_usage_id', $contract->unit_usage_id) == $item->id) selected @endif>{{ $item->name_ar }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="row-state">
        <div class="form-group-state">
            ...
            <select id="unit-number" class="select-state" required name="unit_number">
                ...
                <!--<option value="0" @if(old('unit_number', $contract->unit_number) == 0) selected @endif>لا يوجد</option>-->
                @for ($i = 1; $i <= 50; $i++)
                    <option value="{{ $i }}" @if(old('unit_number', $contract->unit_number) == $i) selected @endif>{{ $i }}</option>
                @endfor
            </select>
        </div>
        <div class="form-group-state">
            ...
            <select id="floor-number" class="select-state" required name="floor_number">
                ...
                @for ($i = 1; $i <= 10; $i++)
                    <option value="{{ $i }}" @if(old('floor_number', $contract->floor_number) == $i) selected @endif>{{ $i }}</option>
                @endfor
            </select>
        </div>
    </div>
    ...
    <div class="form-group-state">
        ...
        <input id="unit-area" class="input-state" value="{{ old('unit_area', $contract->unit_area) }}" placeholder="مساحة الوحدة الإجمالية تقريبا" required name="unit_area">
    </div>
    ...
    <div class="row-state">
        <div class="form-group-state">
            ...
            <select id="rooms-number" class="select-state"   name="tootal_rooms">
                ...
                @for ($i = 1; $i <= 10; $i++)
                    <option value="{{ $i }}" @if(old('tootal_rooms', $contract->tootal_rooms) == $i) selected @endif>{{ $i }}</option>
                @endfor
            </select>
        </div>
        <div class="form-group-state">
            ...
            <select id="halls-number" class="select-state"  name="The_number_of_halls">
                ...
                @for ($i = 1; $i <= 10; $i++)
                    <option value="{{ $i }}" @if(old('The_number_of_halls', $contract->The_number_of_halls) == $i) selected @endif>{{ $i }}</option>
                @endfor
            </select>
        </div>
    </div>
    ...
    <div class="row-state">
        <div class="form-group-state">
            ...
            <select id="kitchens-number" class="select-state"  name="The_number_of_kitchens">
                ...
                @for ($i = 1; $i <= 10; $i++)
                    <option value="{{ $i }}" @if(old('The_number_of_kitchens', $contract->The_number_of_kitchens) == $i) selected @endif>{{ $i }}</option>
                @endfor
            </select>
        </div>
        <div class="form-group-state">
            ...
            <select id="bathrooms-number" class="select-state" name="The_number_of_toilets" >
                ...
                @for ($i = 1; $i <= 10; $i++)
                    <option value="{{ $i }}" @if(old('The_number_of_toilets', $contract->The_number_of_toilets ?? '') == $i) selected @endif>{{ $i }}</option>
                @endfor
            </select>
        </div>
    ...
    <div class="row-state">
        <div class="form-group-state">
            ...
            <select id="window-ac" class="select-state" required name="window_ac">
                ...
                <option value="0" @if(old('window_ac', $contract->window_ac) == 0) selected @endif>لا يوجد</option>
                @for ($i = 1; $i <= 10; $i++)
                    <option value="{{ $i }}" @if(old('window_ac', $contract->window_ac) == $i) selected @endif>{{ $i }}</option>
                @endfor
            </select>
        </div>
        <div class="form-group-state">
            ...
            <select id="split-ac" class="select-state" required name="split_ac">
                ...
                <option value="0" @if(old('split_ac', $contract->split_ac) == 0) selected @endif>لا يوجد</option>
                @for ($i = 1; $i <= 10; $i++)
                    <option value="{{ $i }}" @if(old('split_ac', $contract->split_ac) == $i) selected @endif>{{ $i }}</option>
                @endfor
            </select>
        </div>
    </div>
    ...
    <div class="row-state">
        <div class="form-group-state">
            ...
            <input id="electricity-meter" placeholder="رقم عداد الكهرباء (اختياري)" 
                value="{{ old('electricity_meter_number', $contract->electricity_meter_number) }}"
                class="input-state" name="electricity_meter_number">
        </div>
        <div class="form-group-state">
            <label for="water-meter" class="label-state">رقم عداد المياه (اختياري)</label>
            <input id="water-meter" placeholder="رقم عداد المياه (اختياري)" 
                value="{{ old('water_meter_number', $contract->water_meter_number) }}"
                class="input-state" name="water_meter_number">
        </div>
    </div>
