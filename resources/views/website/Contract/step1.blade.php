...
@section('content')
    ...
    <form action="{{ route('submit.step1', $contract->uuid) }}" method="POST" class="form-content-state">
        @csrf
        ...
        <div class="form-group-state">
            <label for="ownership-state" class="label-state">هل أنت المـالك أو المستاجر؟</label>
              <select id="ownership-state" class="select-state" name="contract_ownership">
                <option value="" hidden disabled selected>هل أنت المـالك أو المستاجر؟</option>
                <option value="1" {{ old('contract_ownership', $contract->contract_ownership) == 'owner' ? 'selected' : '' }}>مالك</option>
                <option value="0" {{ old('contract_ownership', $contract->contract_ownership) == 'tenant' ? 'selected' : '' }}>مستأجر</option>
            </select>
        ...
        <div class="form-group-state">
          ...
          <select id="alive-state" class="select-state" name="property_owner_is_deceased">
                <option value="" hidden disabled selected {{ old('property_owner_is_deceased', $contract->property_owner_is_deceased) === null ? 'selected' : '' }}>هل صاحب العقار حي يرزق؟</option>
                <option value="0" {{ old('property_owner_is_deceased', $contract->property_owner_is_deceased) == '0' ? 'selected' : '' }}>هل صاحب العقار حي يرزق</option>
                <option value="1" {{ old('property_owner_is_deceased', $contract->property_owner_is_deceased) == '1' ? 'selected' : '' }}>نعم، حي يرزق بفضل الله</option>
                <option value="0" {{ old('property_owner_is_deceased', $contract->property_owner_is_deceased) == '3' ? 'selected' : '' }}>لا، متوفي يرحمه الله</option>
            </select>
            ...
            <div class="form-group-state">
                ...
                <select id="property-type" class="select-state" name="property_type_id">
                    ...
                    @foreach ($realTypes as $item)
                        <option value="{{ $item->id }}" {{ old('property_type_id', $contract->property_type_id) == $item->id ? 'selected' : '' }}>
                            {{ $item->name_trans }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group-state">
                ...
                <select id="property-usage" class="select-state" name="property_usages_id">
                    ...
                    @foreach ($usages as $item)
                        <option value="{{ $item->id }}" {{ old('property_usages_id', $contract->property_usages_id) == $item->id ? 'selected' : '' }}>
                            {{ $item->name_trans }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="buttons-state">
            <a type="button" href="{{ route('paperwork',$contract->uuid) }}" class="back-button-state">عودة</a>
