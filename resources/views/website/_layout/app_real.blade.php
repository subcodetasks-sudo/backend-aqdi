...
<body>
  <section class="real-state-form">
    <nav class="nav-menu-state">
      ...
      <!-- Make sure to use the correct parameter name 'id' -->
      <a href="" 
        class="nav-link-state contract-link {{ request()->routeIs('real.submit.step1') ? 'active-state' : '' }}">
        عقدي
     </a>

     <a href="" 
        class="nav-link-state contract-link {{ request()->routeIs('real.submit.step2') ? 'active-state' : '' }}">
        الصك
     </a>

     <a href="" 
        class="nav-link-state contract-link {{ request()->routeIs('real.submit.step3') ? 'active-state' : '' }}">
        المالك
     </a>

     <a href="" 
        class="nav-link-state contract-link {{ request()->routeIs('real.submit.step4') ? 'active-state' : '' }}">
        المستأجر
     </a>

     <a href="" 
        class="nav-link-state contract-link {{ request()->routeIs('real.submit.financial') ? 'active-state' : '' }}">
        البيانات المالية
     </a>
